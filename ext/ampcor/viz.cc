    ...
    py::class_<bmp_t>(pyviz, "BMP", py::buffer_protocol())
        ...
        .def_buffer([](bmp_t & bmp) -> py::buffer_info {
            ...
            return py::buffer_info(
                bmp.data(), sizeof(bmp_t::byte_type),
                py::format_descriptor<bmp_t::byte_type>::format(), 1, { bmp.bytes() }, { 1 });
        })
        // done
        ;
    ...
    pyviz
        .def(
            ...
            [](arena_const_reference arena, int tid) -> bmp_t {
                ...
                // show me
                channel << "tile:" << pyre::journal::newline << "  origin: " << origin
                        << pyre::journal::newline << "  shape: " << shape << pyre::journal::newline
                        << "  viz:" << pyre::journal::newline << "    bins: " << bins
                        << pyre::journal::newline << "    hue: " << hue << pyre::journal::newline
                        << "    saturation: " << saturation << pyre::journal::newline
                        << "    data: [ " << min << ", " << max << "]" << pyre::journal::newline
                        << "    brightness: [ " << minB << ", " << maxB << "]"
                ...
                // make the color map
                arena_uni1d_t cmap(start, bins, hue, saturation, minB, maxB, min, max);
        ...
        .def(
            "slc",
            // the handler
            [](slc_const_reference slc, std::pair<int, int> tileOrigin,
               std::pair<int, int> tileShape, int zoom, std::pair<float, float> range) -> bmp_t {
                // make a channel
                ...
                slc_t::index_type boxOrigin = { scale * tileOrigin.first,
                                                scale * tileOrigin.second };
                ...
                // make the color map
                slc_complex_t cmap(start, bins, saturation, minB, maxB, minv, maxv);
        ...
        .def(
            ...
            [](slc_const_reference slc, std::pair<int, int> tileOrigin,
               std::pair<int, int> tileShape, int zoom, std::pair<float, float> range) -> bmp_t {
                ...
                channel
                    << pyre::journal::newline << "origin: " << boxOrigin << pyre::journal::newline
                    << "shape: " << boxShape << pyre::journal::newline << "step: " << step
                    << pyre::journal::newline << "bins: " << bins << pyre::journal::newline
                    << "hue: " << hue << pyre::journal::newline << "saturation: " << saturation
                    << pyre::journal::newline << "minimum brightness: " << minB
                    << pyre::journal::newline << "maximum brightness: " << maxB
                    << pyre::journal::newline << "minimum value: " << minv << pyre::journal::newline
                    << "maximum value: " << maxv << pyre::journal::newline << pyre::journal::endl;
                // make the color map
                slc_uni1d_t cmap(detector, bins, hue, saturation, minB, maxB, minv, maxv);
        ...
        .def(
            ...
            // the handler
            [](slc_const_reference slc, std::pair<int, int> tileOrigin,
               std::pair<int, int> tileShape, int zoom) -> bmp_t {
                // make a channel
                pyre::journal::debug_t channel("ampcor.viz.slc");
